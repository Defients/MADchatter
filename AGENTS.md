...
### Director Notes (`src/types.ts`, `src/lib/memoryRetrieval.ts`, `src/components/MultiBotPanel.tsx`)
...
- **Store actions:** `addDirectorNote(text, durationMs?)`, `removeDirectorNote(noteId)`, `clearDirectorNotes()` (legacy single-bot); `addBotDirectorNote(botId, text, durationMs?)`, `removeBotDirectorNote(botId, noteId)`, `clearBotDirectorNotes(botId)` (multi-bot). Notes are capped at 50 per bot.
